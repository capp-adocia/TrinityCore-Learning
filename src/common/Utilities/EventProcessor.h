...
class TC_COMMON_API BasicEvent
{
    ...
    public:
        ...
        // 此方法是事件触发时执行的函数，返回false表示事件不希望被删除
        virtual bool Execute(uint64 /*e_time*/, uint32 /*p_time*/) { return true; }

        virtual bool IsDeletable() const { return true; }   // 事件是否可以被安全删除

        virtual void Abort(uint64 /*e_time*/) { }           // 此方法在事件被中止时执行
