...
// 这个是为了方便查询数据，比如查询一个物品的属性，查询一个生物的属性，查询一个游戏对象的属性
enum QueryDataGroup
{
    QUERY_DATA_CREATURES        = 0x01, // 0000 0001
    QUERY_DATA_GAMEOBJECTS      = 0x02, // 0000 0010
    QUERY_DATA_ITEMS            = 0x04, // 0000 0100
    QUERY_DATA_QUESTS           = 0x08, // 0000 1000
    QUERY_DATA_POIS             = 0x10, // 0001 0000

    QUERY_DATA_ALL              = 0xFF  // 1111 1111
};
...
class TC_GAME_API ObjectMgr
{
    ...
    public:
        ...
        {
            std::string_view str = GetLocaleString(data, static_cast<size_t>(localeConstant)); // 获取指定语言字符串
            if (!str.empty()) // 如果字符串存在，则赋值
                value.assign(str);
        }
