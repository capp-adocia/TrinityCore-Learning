...
WorldPacket CreatureTemplate::BuildQueryData(LocaleConstant loc) const
{
    ...
    std::string locName = Name, locTitle = Title; // 将Name和Title转换为locName和locTitle
    ...
    memcpy(queryTemp.Stats.ProxyCreatureID, KillCredit, sizeof(uint32) * MAX_KILL_CREDIT); // 复制KillCredit数组
    ...
    queryTemp.Write(); // 写入数据包
    queryTemp.ShrinkToFit(); // 优化数据包大小
    return queryTemp.Move(); // 返回数据包，并转移所有权
}
