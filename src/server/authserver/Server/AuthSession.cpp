...
{
    if (!AuthSocket::Update()) // 这个更新指的是将socket中的数据写入到输出网络缓存区中
        return false;
...
void AuthSession::CheckIpCallback(PreparedQueryResult result)
{
    if (result)
    {
        ...
        do
        {
            ...
            if (fields[0].GetUInt64() != 0) // 如果存在被禁止的IP地址
                banned = true;
...
void AuthSession::ReadHandler()
{
    ...
    {
        uint8 cmd = packet.GetReadPointer()[0]; // 读取数据包的第一个字节，即命令
        auto itr = Handlers.find(cmd); // 在Handlers中查找这个命令
        // 处理头部
        if (itr == Handlers.end())
        {
            // 好吧，我们没有处理这个，让我们忽略它
        ...
        // 也许是客户端这时候该关闭连接了
        if (_status != itr->second.status) // 如果状态不匹配，则关闭连接
        {
            CloseSocket();
            return;
        }

        uint16 size = uint16(itr->second.packetSize); // 获取数据包的大小
        ...
        if (cmd == AUTH_LOGON_CHALLENGE || cmd == AUTH_RECONNECT_CHALLENGE)
        {
            ...
            size += challenge->size;
            // 如果经过扩展后的大小超过了允许的最大挑战大小（MAX_ACCEPTED_CHALLENGE_SIZE），则关闭连接并返回。
        ...
        if (packet.GetActiveSize() < size) // 如果数据包大小小于实际大小，则跳出循环
            // 等待
            break;

        if (!(*this.*itr->second.handler)()) // 如果处理函数返回false，则关闭连接
        {
            CloseSocket();
            return;
        }
    ...
    AsyncRead(); // 里面会涉及到将缓冲区重置后，写操作可能完成了，然后递归调用ReadHandler
}

void AuthSession::SendPacket(ByteBuffer& packet) // 发送数据包
{
    if (!IsOpen())
        return;
    ...
    if (!packet.empty())
    {
        ...
        buffer.Write(packet.contents(), packet.size()); // 将数据包写入缓冲区
        QueuePacket(std::move(buffer)); // 将缓冲区加入队列
        // 等待发送
    }
}
...
void AuthSession::LogonChallengeCallback(PreparedQueryResult result) // 这里应该是笔误
{
    ByteBuffer pkt;
    pkt << uint8(AUTH_LOGON_CHALLENGE);
    pkt << uint8(0x00);
...
void AuthSession::ReconnectChallengeCallback(PreparedQueryResult result)
{
    ...
    // std::array<uint8, 16> _reconnectProof = {}; 存储连接证明
