...
QueryCallback DatabaseWorkerPool<T>::AsyncQuery(char const* sql)
{
    ...
    // 需要在入队之前存储 future result 任务可能在返回此方法之前被处理和删除
...
QueryCallback DatabaseWorkerPool<T>::AsyncQuery(PreparedStatement<T>* stmt)
{
    ...
    Enqueue(task);
    return QueryCallback(std::move(result)); // 返回 QueryCallback 对象 数据都存储在这个对象里面
}
...
void DatabaseWorkerPool<T>::KeepAlive()
{
    ...
    // 假设所有工作线程都空闲，每个工作线程将接收1个ping操作请求
    // 如果一个或多个工作线程忙碌，ping操作将不会均匀分配，但这并不重要，因为唯一目的是防止连接空闲
...
T* DatabaseWorkerPool<T>::GetFreeConnection()
{
    ...
    for (;;)
    {
        ...
        if (connection->LockIfReady()) // 尝试获取锁
            break;
    }
...
void DatabaseWorkerPool<T>::Execute(char const* sql)
{
    ...
    BasicStatementTask* task = new BasicStatementTask(sql); // 同步的任务
    Enqueue(task);
}
...
void DatabaseWorkerPool<T>::DirectExecute(char const* sql)
{
    ...
    T* connection = GetFreeConnection(); // 这里会加锁
