...
// 这个是一个宏定义，用于定义一个枚举类型WorldDatabaseStatements，其中包含了多个枚举值，每个枚举值对应一个数据库操作语句的ID。
enum WorldDatabaseStatements : uint32
{
    ...
    */
    ...
    WORLD_DEL_LINKED_RESPAWN, // 这个是一个枚举值，对应一个数据库操作语句的ID，用于删除一个关联的复活点。
    WORLD_DEL_LINKED_RESPAWN_MASTER, // 这个是一个枚举值，对应一个数据库操作语句的ID，用于删除一个关联的复活点的主节点。
...
public:
    ...
    //- Constructors for sync and async connections // 同步和异步连接的构造函数
    WorldDatabaseConnection(MySQLConnectionInfo& connInfo); // 同步连接
    WorldDatabaseConnection(ProducerConsumerQueue<SQLOperation*>* q, MySQLConnectionInfo& connInfo); // 异步连接
