...
int MySQLConnection::ExecuteTransaction(std::shared_ptr<TransactionBase> transaction)
{
    ...
    // 在某些查询中，我们可能会遇到错误，并且根据错误类型，我们可能需要重启事务。
    // 为了防止数据丢失，我们只在所有操作完成后清理。
    // 这是在调用函数DatabaseWorkerPool<T>:DirectCommitTransaction和TransactionTask::Execute时完成的，而不是在遍历每个元素时完成的。
